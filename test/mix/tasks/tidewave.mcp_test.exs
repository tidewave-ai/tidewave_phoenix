defmodule Mix.Tasks.Tidewave.McpTest do
  use ExUnit.Case, async: false

  @moduletag :capture_log

  test "serves MCP requests over stdio" do
    output =
      run_task([
        %{
          "jsonrpc" => "2.0",
          "method" => "initialize",
          "id" => 1,
          "params" => %{"protocolVersion" => "2025-03-26"}
        },
        %{"jsonrpc" => "2.0", "method" => "notifications/initialized"},
        %{
          "jsonrpc" => "2.0",
          "method" => "tools/call",
          "id" => 2,
          "params" => %{
            "name" => "project_eval",
            "arguments" => %{"code" => "1 + 1"}
          }
        }
      ])

    assert [initialize, tool_call] = decode_lines(output)

    assert initialize["id"] == 1
    assert initialize["result"]["protocolVersion"] == "2025-03-26"

    refute "browser_eval" in Enum.map(initialize["result"]["tools"], & &1["name"])

    assert tool_call["id"] == 2
    assert tool_call["result"]["content"] == [%{"type" => "text", "text" => "2"}]
  end

  defp run_task(messages) do
    input = Enum.map_join(messages, "\n", &Jason.encode!/1)
    {:ok, stdio} = StringIO.open(input <> "\n")
    original_group_leader = Process.group_leader()
    Process.group_leader(self(), stdio)

    try do
      Mix.Task.reenable("tidewave.mcp")
      assert :ok = Mix.Task.run("tidewave.mcp", ["--no-logger-redirect"])
      {_input, output} = StringIO.contents(stdio)
      output
    after
      Process.group_leader(self(), original_group_leader)
      StringIO.close(stdio)
    end
  end

  defp decode_lines(output) do
    output
    |> String.split("\n", trim: true)
    |> Enum.map(&Jason.decode!/1)
  end
end
